...
func _generate_procedural_map():
	...
	ts_source.texture = preload("res://Assets/Images/tileset_nature.png")
	ts_source.texture_region_size = Vector2i(32, 32)
	...
	player.get_node("Camera2D").limit_right = map_w * 32
	player.get_node("Camera2D").limit_bottom = map_h * 32
	...
	for x in range(map_w):
		for y in range(map_h):
			...
			# 테두리는 나무 덤불 무작위로 감싸서 숲 느낌
			if x == 0 or x == map_w - 1 or y == 0 or y == map_h - 1:
				tile_x = randi_range(3, 5) # 울타리/나무/덤불 타일 지역(추정)
				tile_y = 1
			...
			if x > 2 and x < map_w - 2 and y > 2 and y < map_h - 2:
				if randf() < 0.05:
					tilemap.set_cell(Vector2i(x, y), 0, Vector2i(randi_range(0,2), 1)) # 꽃밭 장식
...
func spawn_wild_plants():
	...
	for i in range(plant_count):
		...
		var px = randf_range(3 * 32, (map_w - 3) * 32)
		var py = randf_range(3 * 32, (map_h - 3) * 32)
